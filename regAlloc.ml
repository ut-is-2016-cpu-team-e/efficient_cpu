...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  | Nop | Li _ | SetL _ | SetExt _ | Comment _ | Restore _ | FLi _ as exp -> (Ans(exp), regenv)
  ...
  | ShiftL2(x) -> (Ans(ShiftL2(find x Type.Int regenv)), regenv)
  | ShiftR1(x) -> (Ans(ShiftR1(find x Type.Int regenv)), regenv)
