...
let rec recursive x = function
  | IfEq(_, _, e1, e2) | IfLE(_, _, e1, e2) | IfFAbsLE(_, _, e1, e2) -> (recursive x e1) || (recursive x e2)
  | Let(_, e1, e2) | LetRec({body = e1}, e2) -> (recursive x e1) || (recursive x e2)
  | LetTuple(_, _, e) -> recursive x e
  | App(y, _) | App2(y, _) -> if x = y then true else false
  | _ -> false

let rec size = function
  | IfEq(_, _, e1, e2) | IfLE(_, _, e1, e2) | IfFAbsLE(_, _, e1, e2)
...
let rec g env = function (* インライン展開ルーチン本体 (caml2html: inline_g) *)
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) -> (* 関数定義の場合 (caml2html: inline_letrec) *)
      let env =
      let threshold2 = !threshold / 4 in
      if (size e1 > !threshold) || ((recursive x e1) && (size e1 > threshold2)) then env else M.add x (yts, e1) env in
