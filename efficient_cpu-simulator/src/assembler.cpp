...
Asm::Asm(const char* filename) : isJoutOn(true), isDumpOn(true) {
  ...
  cout << string(70, '-') << endl << "<Instruction>" << endl;
...
void Asm::regHandler(string& reg) {
  if (gReg2bin.count(reg)) {
    reg = gReg2bin[reg];
  } else {
    cerr << "Register : " << reg << " has some thing wrong." << endl;
    assert(false);
  }
}
...
string Asm::assemble(string& cmd, const int addr, unordered_map<string, int>& label) {
  ...
  if (op == "nop") {
   return "00000000000000000000000000000000";
    ...
    return "11000100000000000000000000000000";
  }

  if (gAsmb.count(op)) {
    as = gAsmb[op]; // gAsmb[hoge] has been defined in setInit()
  } else {
    cerr << "Opcode: " << op << " can't be handled." << endl;
    assert(false);
  }
  ...
  for (int i = 0; i < as.mOperandNum; i++) {
    ...
    } /* else if (as.mRegNum == -2) {

      if (i == 0) {
        r[i] = gReg2bin[r[i]];
      } else if (i == 1) {
        divideRegOff(r[i], r[i+1]); // r[i]: rs, r[i+1]: offset
      }
    ...
    else if (as.mFormat == "B" && op == "bni") {

      if (i == 2) {
        assert(label.count(r[i]));
        r[i] = i2bin(label[r[i]] - addr - 1, 16);
      }
      else if (i == 1) {
        dec2bin(r[i], 5);
      }
    ...
    else if (as.mFormat == "B") { // beq, bne, blt, fbeq, fbne, fblt, bltfabs
    ...
    else if (as.mFormat == "J" && op != "lil") { // j, jal, push
    ...
    else if (i < as.mRegNum) { // add, sub, mul, addi, and, andi, or, xor, ori, (lti), jr, move,
      //subi, li, fadd, fsub, fmul, fdiv, fmadd, fmsub, finv, fabs, fmove, mfc1, mtc1

      regHandler(r[i]);

    }
    else { // addi, andi, ori, lti, subi, li, flihi, flilo
  ...
  if (as.mFormat == "R") {
    ...
    else if (as.mOperandNum == 4) {
      return as.mOpcd + r[1] + r[2] + r[0] + r[3] + as.mFunct; // opcode $rd $rs $rt $r
    }

  }
  else if (as.mFormat == "I") {

    if (op == "fmove" || op == "mfc1" || op == "mtc1") {
      return as.mOpcd + r[1] + r[0] + "00000" + as.mShamt + as.mFunct;
    }
    else if (op == "print_int" || op == "print_char" || op == "read_int" || op == "read_float") {
      return as.mOpcd + r[0] + "0000000000" + as.mShamt + as.mFunct;
    }
    else if (op == "lw" || op == "sw" || op == "flw" || op == "fsw") {
      return as.mOpcd + r[1] + r[2] + r[0] + r[3];
    }
    else if (op != "li" && op != "flihi" && op != "flilo") {
    ...
    cerr << "Instruction : " << cmd << " has something wrong." << endl;
    assert(false);
...
void Asm::setInit() {
  ...
  gAsmb["mul"] = AsmT(3, 3, "R", "000000", "00000", "100001");
  gAsmb["addi"] = AsmT(2, 3, "I", "001000", "", "");
  gAsmb["lw"] = AsmT(-3, 4, "I", "100011", "", "");
  gAsmb["sw"] = AsmT(-3, 4, "I", "101011", "", "");
  ...
  gAsmb["sl2add"] = AsmT(3, 3, "R", "000000", "00000", "000101");
  ...
  gAsmb["flihi"] = AsmT(1, 2, "I", "011000", "", "");
  gAsmb["flilo"] = AsmT(1, 2, "I", "011001", "", "");
  gAsmb["fneg"] = AsmT(2, 2, "R", "010000", "00000", "001011");

  gAsmb["li"] = AsmT(1, 2, "I", "000001", "", "");
  gAsmb["lil"] = AsmT(1, 2, "J", "001001", "", "");
  // library
  gAsmb["fsqrt"] = AsmT(2, 2, "R", "010000", "00000", "001100");
