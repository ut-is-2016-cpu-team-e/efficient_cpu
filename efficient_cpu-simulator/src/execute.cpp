...
void Exec::executeF() {
  ...
  if (mOpCode == 16) {
    ...
    case 0x15 : mShamt = bitRange(mInst, 6, 10);
                ...
                break;
    // fmsub
    case 0x16 : mShamt = bitRange(mInst, 6, 10);
                mFrs = mFrs * mFrt;
                mU.reg = cpu(mShamt);
                mU.freg = mFrs - mU.freg;
                cpu(mRd) = mU.reg;
                cpu.incInstCnt(Cpu::NUM_FMSUB);
                break;
    // finv
    ...
    // flw
    mImm = bitRange(mInst, 0, 10);
    if (mImm & (1 << 10)) {
      mImm |= 0xfffff800;
    }
    cpu(mRd) = cpu.getRam(cpu[mRs]+cpu[mRt]+mImm);
    /*mvprintw(mY-1, 0, "flw: freg[%d] <-- mem[%d]", mRd, cpu[mRs]+cpu[mRt]+mImm); */
    ...
    cpu.getRam(cpu[mRs]+cpu[mRt]+mImm) = cpu(mRd);
    /*mvprintw(mY-1, 0, "fsw: freg[%d] --> mem[%d]", mRd, cpu[mRs]+cpu[mRt]+mImm); */
    ...
    // flilo
    cpu(mRt) = (cpu(mRt) & 0xffff0000) | mImm;
...
void Exec::execRom() {
  ...
  case 0x08 : extendSign16(mImm);
              ...
              break;
    // lw
  case 0x23 : mRd = bitRange(mInst, 11, 15);
              mImm = bitRange(mInst, 0, 10);
              if (mImm & (1 << 10)) {
                mImm |= 0xfffff800;
              }
              cpu[mRd] = cpu.getRam(cpu[mRs] + cpu[mRt] + mImm);
              /*mvprintw(mY-1, 0, "lw: reg[%d] <-- mem[%d]", mRd, cpu[mRs]+cpu[mRt]+mImm); */
              cpu.incInstCnt(Cpu::NUM_LW);
              break;
    // sw
  case 0x2b : mRd = bitRange(mInst, 11, 15);
              mImm = bitRange(mInst, 0, 10);
              if (mImm & (1 << 10)) {
                mImm |= 0xfffff800;
              }
              cpu.getRam(cpu[mRs] + cpu[mRt] + mImm) = cpu[mRd];
              /*mvprintw(mY-1, 0, "sw: reg[%d] --> mem[%d]", mRd, cpu[mRs]+cpu[mRt]+mImm); */
  ...
  case 0x03 : mAddress = bitRange(mInst, 0, 25);
              ...
              break;
    // push
  case 0x30 : mAddress = bitRange(mInst, 0, 25);
              cpu.push(cpu.getPc());
              cpu.getPc() = mAddress;
              cpu.mLabelCnt[mAddress]++;
              /*mvprintw(mY-1, 0, "address stack size: %d", cpu.size()); */
              cpu.incInstCnt(Cpu::NUM_PUSH);
              break;
    // pop
  case 0x31 : cpu.getPc() = cpu.pop();
              /*mvprintw(mY-1, 0, "address stack size: %d", cpu.size()); */
              cpu.incInstCnt(Cpu::NUM_POP);
              break;
    // lil
