...
void executeF(Cpu& cpu) {
  ...
  if (opCode == 16) {
    ...
    while (1) {
      ...
      cin >> floatVal;
      break;
      /*if (isDigit(floatVal)) { break; }
      else { cout << floatVal << " is not a number." << endl; }*/
    }
...
void execRom(Cpu& cpu) {
  ...
  case 0x01 : extendSign16(imm);
              ...
              break;
    // print_int
  case 0x20 : cout << "print_int :" << cpu[rs] << " >> \"sim.ppm\"" << endl;
              fprintf(cpu.ppm, "%d", cpu[rs]);
              fflush(cpu.ppm);
  ...
  case 0x21 : while (1) {
                ...
                cin >> intVal;
                break;
                /*if (isDigit(intVal)) { break; }
                else { cout << intVal << " is not a number." << endl; }*/
              }
              cpu[1] = stoi(intVal);
              cpu.incInstCnt(Cpu::NUM_READ_INT);
              break;
    // print_byte
  case 0x22 : cout << "print_byte :" << cpu[rs] << " >> \"sim.ppm\"" << endl;
              fprintf(cpu.ppm, "%c", cpu[rs]);
              fflush(cpu.ppm);
              cpu.incInstCnt(Cpu::NUM_PRINT_BYTE);
              break;
    // hlt
